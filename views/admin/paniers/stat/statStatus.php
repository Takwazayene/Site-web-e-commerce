...
<?php
   ...
   // $strQuery = "SELECT nom, qte FROM produit ORDER BY qte DESC LIMIT 10";
        $strQuery = "SELECT status, count(id) as quantite FROM reservation GROUP BY status ORDER BY count(id) DESC LIMIT 10";
    ...
    if ($result) {
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "organisation des status des reservations par ordre decroissant",
